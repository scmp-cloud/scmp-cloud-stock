...
public class ImageUtils {
	...
	public static void mergeImage(String[] files, int type, String targetFile) {
		...
		try {
			...
			for (int i = 0; i < images.length; i++) {
				if (type == 1) {
					mergedImage.setRGB(width_i, 0, images[i].getWidth(), newHeight, ImageArrays[i], 0, images[i].getWidth());
					width_i += images[i].getWidth();
				} else if (type == 2) {
					mergedImage.setRGB(0, height_i, newWidth, images[i].getHeight(), ImageArrays[i], 0, newWidth);
					height_i += images[i].getHeight();
				}
			}
			//输出想要的图片
			ImageIO.write(mergedImage, "png", new File(targetFile));
